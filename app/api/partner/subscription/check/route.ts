...
export async function GET() {
  try {
    ...
    if (!subscriptionInfo) {
      const isGest = sessionUser.mallUserId?.toLowerCase().startsWith('gest') || sessionUser.phone?.toLowerCase().startsWith('gest');
      if (isGest) {
        try {
          ...
          if (!existingContract) {
            ...
            await prisma.affiliateContract.create({
              data: {
                ...
                email: `${sessionUser.mallUserId || sessionUser.phone}@example.com`,
                address: '테스트 주소',
