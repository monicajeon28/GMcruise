...
export async function POST(req: Request) {
  try {
    ...
    if (!payappUserid || !payappLinkkey) {
      console.error('[Subscription Payment] PayApp 설정 누락');
      return NextResponse.json(
    ...
    const params = {
      cmd: 'payrequest',
      userid: payappUserid,
      goodname: `${goodname} - ${sessionUser.name || sessionUser.mallUserId || '정액제 판매원'}`,
      price: price,
      ...
      smsuse: 'n', // SMS 발송 안함
      returnurl: `${baseUrl}/partner/${sessionUser.mallUserId || sessionUser.phone}/dashboard?payment=success`,
      openpaytype: 'card', // 카드번호 입력 결제만
      checkretry: 'y', // feedbackurl 재시도
      skip_cstpage: 'y', // 매출전표 페이지 이동 안함
    };

    console.log('[Subscription Payment] 결제 요청 파라메터:', {
      cmd: params.cmd,
    ...
    console.log('[Subscription Payment] PayApp 응답:', result);

    if (result.state === '1') {
      // 결제 요청 성공
      console.log('[Subscription Payment] 결제 요청 성공:', {
      ...
      // 결제 요청 실패
      console.error('[Subscription Payment] 결제 요청 실패:', {
        state: result.state,
