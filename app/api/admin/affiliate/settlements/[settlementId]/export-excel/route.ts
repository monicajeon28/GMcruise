...
export async function GET(
  ...
  try {
    ...
    for (const entry of settlement.ledgerEntries) {
      ...
      // 대리점장 수당 계산 (오버라이드)
      if (entry.entryType === 'OVERRIDE_COMMISSION' && sale.AffiliateProfile_AffiliateSale_managerIdToAffiliateProfile) {
      ...
      if (!existingSale) {
        ...
        const managerCommission = saleLedgers.find(e => e.entryType === 'OVERRIDE_COMMISSION' && sale.AffiliateProfile_AffiliateSale_managerIdToAffiliateProfile && e.profileId === sale.AffiliateProfile_AffiliateSale_managerIdToAffiliateProfile.id)?.amount || 0;
        const managerWithholding = saleLedgers.find(e => e.entryType === 'OVERRIDE_COMMISSION' && sale.AffiliateProfile_AffiliateSale_managerIdToAffiliateProfile && e.profileId === sale.AffiliateProfile_AffiliateSale_managerIdToAffiliateProfile.id)?.withholdingAmount || 0;
