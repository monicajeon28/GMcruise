# "지니야 가자" 키워드 검색 기능 점검 보고서

> **작성일**: 2025-01-23  
> **문제**: 현위치/지정위치에서 "맛집" 등 키워드 검색 시 네비게이션만 제공되고 주변 검색 결과가 나오지 않음  
> **목적**: 원인 파악 및 해결 방안 제시

---

## 📋 목차

1. [문제 상황](#문제-상황)
2. [원인 분석](#원인-분석)
3. [현재 코드 구조](#현재-코드-구조)
4. [해결 방안](#해결-방안)
5. [구현 계획](#구현-계획)

---

## 문제 상황

### 사용자 시나리오

**입력 예시:**
- "홍콩에서 맛집"
- "현위치에서 관광지"
- "도쿄에서 카페"

**기대 동작:**
- 주변 맛집/관광지/카페를 보여주는 Google Maps 검색 결과 지도가 펼쳐져야 함

**실제 동작:**
- 네비게이션(길찾기) 링크만 제공됨
- "홍콩"에서 "맛집"으로 가는 길찾기 링크가 생성됨
- 주변 검색 결과가 표시되지 않음

---

## 원인 분석

### 1. 키워드 감지 로직 부재

**현재 코드 흐름:**

```26:57:app/api/chat/route.ts
    // go 모드: 경로 안내
    if (mode === 'go') {
      let messages: ChatMessage[];
      
      if (from && to) {
        // from/to가 이미 파싱된 경우 직접 사용 (불필요한 변환 제거)
        const origin = { text: from };
        const dest = { text: to };
        const urls = buildAllDirUrls(origin, dest);
        
        messages = [
          { type:'text', text:`확인했어요.\n출발지: ${from}\n도착지: ${to}` },
          {
            type:'map-links',
            title:'길찾기',
            links:[
              { label:'🚗 자동차 길찾기', href: urls.driving, kind: 'directions' },
              { label:'🚇 대중교통 길찾기', href: urls.transit, kind: 'directions' },
              { label:'🚶 도보 길찾기', href: urls.walking, kind: 'directions' },
            ],
          },
          { type:'text', text:'새 창에서 열려요. 지도에서 **시작**만 누르시면 됩니다.' }
        ];
      } else {
        // text에서 파싱 (resolveFromTo 사용)
        messages = handleDirections(text);
      }
      
      return NextResponse.json({ ok: true, messages });
    }
```

**문제점:**
- ❌ 키워드(맛집, 관광지, 카페 등) 감지 로직이 없음
- ❌ 모든 입력을 길찾기(directions)로 처리
- ❌ 검색(search) 기능이 호출되지 않음

---

### 2. 기존 키워드 감지 코드는 있으나 미사용

**존재하는 코드:**

```120:124:app/api/chat/detect.ts
// 근처 키워드
export const NEARBY_KEYWORDS = ["스타벅스", "카페", "편의점", "마트", "market", "식당", "약국", "호텔"];
export function extractNearbyKeyword(t: string) {
  return NEARBY_KEYWORDS.find(k => t.toLowerCase().includes(k.toLowerCase()));
}
```

**문제점:**
- ✅ `extractNearbyKeyword()` 함수는 존재함
- ❌ `app/api/chat/route.ts`에서 사용되지 않음
- ❌ `handleDirections()`에서도 사용되지 않음

---

### 3. 검색 URL 생성 함수는 존재

**존재하는 코드:**

```19:22:lib/maps.ts
export function gmSearchUrl(q: string | LatLng) {
  const val = typeof q === 'string' ? encodeURIComponent(q) : `${q.lat},${lng}`;
  return `https://www.google.com/maps/search/?api=1&query=${val}`;
}
```

**문제점:**
- ✅ `gmSearchUrl()` 함수는 존재함
- ❌ 키워드 검색 시 호출되지 않음

---

## 현재 코드 구조

### 1. API 라우트 (`app/api/chat/route.ts`)

```
사용자 입력: "홍콩에서 맛집"
    ↓
mode === 'go' 확인
    ↓
from && to 확인
    ↓
없으면 → handleDirections(text) 호출
    ↓
resolveFromTo("홍콩에서 맛집") 파싱
    ↓
"홍콩" → 출발지
"맛집" → 도착지 (❌ 문제!)
    ↓
길찾기 링크 생성 (❌ 검색이 아님!)
```

### 2. 핸들러 함수 (`app/api/chat/handlers/directions.ts`)

```4:30:app/api/chat/handlers/directions.ts
export function handleDirections(text: string): ChatMessage[] {
  const parsed = resolveFromTo(text);
  
  if (!parsed) {
    return [{ 
      type:'text', 
      text:'입력 형식을 확인해 주세요.\n\n✅ 지원되는 형식:\n• "A에서 B까지"\n• "A부터 B까지"\n• "A → B" (화살표)\n• "A to B" (영어)\n• "마이애미" (지명만 입력 시 공항→터미널 자동 추론)\n\n📝 예시:\n• "홍콩 공항에서 카이탁 터미널까지"\n• "마이애미 공항 → 마이애미 크루즈 터미널"\n• "마이애미" (자동으로 공항→터미널 경로 찾기)' 
    }];
  }
  
  const { origin, dest, originText, destText } = parsed;
  const urls = buildAllDirUrls(origin, dest);
  
  return [
    { type:'text', text:`확인했어요.\n출발지: ${originText}\n도착지: ${destText}` },
    {
      type:'map-links',
      title:'길찾기',
      links:[
        { label:'🚗 자동차 길찾기', href: urls.driving, kind: 'directions' },
        { label:'🚇 대중교통 길찾기', href: urls.transit, kind: 'directions' },
        { label:'🚶 도보 길찾기', href: urls.walking, kind: 'directions' },
      ],
    },
    { type:'text', text:'새 창에서 열려요. 지도에서 **시작**만 누르시면 됩니다.' }
  ];
}
```

**문제점:**
- ❌ 키워드 감지 없음
- ❌ 모든 입력을 출발지/도착지로 파싱
- ❌ 검색 링크 생성 없음

---

## 해결 방안

### 1. 키워드 감지 로직 추가

**확장된 키워드 목록:**
```typescript
const SEARCH_KEYWORDS = [
  "맛집", "식당", "restaurant", "food",
  "관광지", "tourist", "attraction", "sightseeing",
  "카페", "cafe", "coffee",
  "편의점", "convenience", "store",
  "마트", "market", "supermarket",
  "약국", "pharmacy", "drugstore",
  "호텔", "hotel",
  "은행", "bank",
  "병원", "hospital",
  "주유소", "gas station",
  "ATM"
];
```

### 2. 검색 vs 길찾기 구분 로직

**로직 흐름:**
```
입력: "홍콩에서 맛집"
    ↓
키워드 감지: "맛집" 발견
    ↓
출발지 추출: "홍콩"
    ↓
검색 쿼리 생성: "홍콩 맛집" 또는 "맛집 near 홍콩"
    ↓
gmSearchUrl() 호출
    ↓
검색 링크 제공
```

### 3. 구현 위치

**옵션 1: `handleDirections()` 함수 내부 수정**
- 장점: 기존 구조 유지
- 단점: 함수 이름과 기능이 맞지 않음

**옵션 2: 새로운 함수 `handleKeywordSearch()` 생성**
- 장점: 관심사 분리, 명확한 구조
- 단점: 추가 함수 필요

**옵션 3: `route.ts`에서 사전 처리**
- 장점: 핸들러 함수는 순수하게 유지
- 단점: 라우트 파일이 복잡해짐

**권장: 옵션 2 + 옵션 3 조합**
- `route.ts`에서 키워드 감지
- 키워드가 있으면 `handleKeywordSearch()` 호출
- 키워드가 없으면 기존 `handleDirections()` 호출

---

## 구현 계획

### Phase 1: 키워드 감지 함수 개선

**파일:** `app/api/chat/detect.ts`

```typescript
// 기존 코드 확장
export const SEARCH_KEYWORDS = [
  ...NEARBY_KEYWORDS,
  "맛집", "식당", "restaurant", "food",
  "관광지", "tourist", "attraction", "sightseeing",
  "은행", "bank",
  "병원", "hospital",
  "주유소", "gas station",
  "ATM"
];

export function extractSearchKeyword(t: string): string | null {
  return SEARCH_KEYWORDS.find(k => t.toLowerCase().includes(k.toLowerCase())) || null;
}

export function isKeywordSearch(text: string): boolean {
  return extractSearchKeyword(text) !== null;
}
```

---

### Phase 2: 키워드 검색 핸들러 생성

**파일:** `app/api/chat/handlers/keyword-search.ts` (신규)

```typescript
import { gmSearchUrl } from '@/lib/maps';
import { extractSearchKeyword, parseOriginDestination } from '@/app/api/chat/detect';
import type { ChatMessage } from '@/lib/chat-types';

export function handleKeywordSearch(text: string): ChatMessage[] {
  const keyword = extractSearchKeyword(text);
  if (!keyword) {
    return [{ 
      type: 'text', 
      text: '키워드를 찾을 수 없습니다. 맛집, 관광지, 카페 등을 입력해 주세요.' 
    }];
  }

  // 출발지 추출
  const { originText, destText } = parseOriginDestination(text);
  const location = originText || '현재 위치';
  
  // 검색 쿼리 생성
  const searchQuery = originText 
    ? `${originText} ${keyword}` 
    : keyword;
  
  const searchUrl = gmSearchUrl(searchQuery);
  
  return [
    { 
      type: 'text', 
      text: `${location} 근처 **${keyword}** 검색 결과를 준비했습니다.` 
    },
    {
      type: 'map-links',
      title: `주변 ${keyword} 검색`,
      links: [
        { 
          label: `🔎 ${location} 근처 ${keyword} 찾기`, 
          href: searchUrl, 
          kind: 'search' 
        },
      ],
    },
    { 
      type: 'text', 
      text: '버튼을 누르면 새 창에서 주변 검색 결과가 열립니다.' 
    }
  ];
}
```

---

### Phase 3: 라우트 수정

**파일:** `app/api/chat/route.ts`

```typescript
import { handleKeywordSearch } from './handlers/keyword-search';
import { isKeywordSearch } from '@/app/api/chat/detect';

// go 모드: 경로 안내
if (mode === 'go') {
  let messages: ChatMessage[];
  
  // 키워드 검색인지 확인
  if (isKeywordSearch(text)) {
    messages = handleKeywordSearch(text);
  } else if (from && to) {
    // 기존 로직 (길찾기)
    // ...
  } else {
    messages = handleDirections(text);
  }
  
  return NextResponse.json({ ok: true, messages });
}
```

---

## 상세 구현 코드

### 1. 키워드 감지 함수 확장

**파일:** `app/api/chat/detect.ts`

```typescript
// 기존 NEARBY_KEYWORDS 확장
export const SEARCH_KEYWORDS = [
  "스타벅스", "카페", "편의점", "마트", "market", "식당", "약국", "호텔",
  "맛집", "restaurant", "food", "dining",
  "관광지", "tourist", "attraction", "sightseeing", "landmark",
  "은행", "bank",
  "병원", "hospital", "clinic",
  "주유소", "gas station", "gasoline",
  "ATM", "atm"
];

export function extractSearchKeyword(t: string): string | null {
  const lower = t.toLowerCase();
  return SEARCH_KEYWORDS.find(k => lower.includes(k.toLowerCase())) || null;
}

export function isKeywordSearch(text: string): boolean {
  return extractSearchKeyword(text) !== null;
}
```

---

### 2. 키워드 검색 핸들러

**파일:** `app/api/chat/handlers/keyword-search.ts` (신규 생성)

```typescript
import { gmSearchUrl } from '@/lib/maps';
import { extractSearchKeyword, parseOriginDestination } from '@/app/api/chat/detect';
import type { ChatMessage } from '@/lib/chat-types';

export function handleKeywordSearch(text: string, from?: string, to?: string): ChatMessage[] {
  // 키워드 추출
  const keyword = extractSearchKeyword(text);
  if (!keyword) {
    return [{ 
      type: 'text', 
      text: '키워드를 찾을 수 없습니다. 맛집, 관광지, 카페 등을 입력해 주세요.' 
    }];
  }

  // 출발지 추출 (from 파라미터 우선, 없으면 text에서 파싱)
  let location = from;
  if (!location) {
    const parsed = parseOriginDestination(text);
    location = parsed.originText || '현재 위치';
  }
  
  // 검색 쿼리 생성
  // 예: "홍콩 맛집" 또는 "맛집 near 홍콩"
  const searchQuery = location && location !== '현재 위치'
    ? `${location} ${keyword}`
    : keyword;
  
  const searchUrl = gmSearchUrl(searchQuery);
  
  // 키워드 한글 표시
  const keywordDisplay = keyword === 'restaurant' ? '맛집' :
                        keyword === 'cafe' ? '카페' :
                        keyword === 'tourist' ? '관광지' :
                        keyword;
  
  return [
    { 
      type: 'text', 
      text: `${location} 근처 **${keywordDisplay}** 검색 결과를 준비했습니다.` 
    },
    {
      type: 'map-links',
      title: `주변 ${keywordDisplay} 검색`,
      links: [
        { 
          label: `🔎 ${location} 근처 ${keywordDisplay} 찾기`, 
          href: searchUrl, 
          kind: 'search' 
        },
      ],
    },
    { 
      type: 'text', 
      text: '버튼을 누르면 새 창에서 주변 검색 결과가 열립니다. 지도에서 여러 결과를 확인할 수 있어요.' 
    }
  ];
}
```

---

### 3. 라우트 수정

**파일:** `app/api/chat/route.ts`

```typescript
import { handleKeywordSearch } from './handlers/keyword-search';
import { isKeywordSearch } from '@/app/api/chat/detect';

// go 모드: 경로 안내
if (mode === 'go') {
  let messages: ChatMessage[];
  
  // 키워드 검색인지 확인
  if (isKeywordSearch(text)) {
    messages = handleKeywordSearch(text, from, to);
  } else if (from && to) {
    // 기존 로직 (길찾기)
    const origin = { text: from };
    const dest = { text: to };
    const urls = buildAllDirUrls(origin, dest);
    
    messages = [
      { type:'text', text:`확인했어요.\n출발지: ${from}\n도착지: ${to}` },
      {
        type:'map-links',
        title:'길찾기',
        links:[
          { label:'🚗 자동차 길찾기', href: urls.driving, kind: 'directions' },
          { label:'🚇 대중교통 길찾기', href: urls.transit, kind: 'directions' },
          { label:'🚶 도보 길찾기', href: urls.walking, kind: 'directions' },
        ],
      },
      { type:'text', text:'새 창에서 열려요. 지도에서 **시작**만 누르시면 됩니다.' }
    ];
  } else {
    // text에서 파싱 (resolveFromTo 사용)
    messages = handleDirections(text);
  }
  
  return NextResponse.json({ ok: true, messages });
}
```

---

## 테스트 시나리오

### 시나리오 1: 키워드 검색 (맛집)

**입력:** "홍콩에서 맛집"

**기대 결과:**
```
확인 메시지: "홍콩 근처 **맛집** 검색 결과를 준비했습니다."
링크: "🔎 홍콩 근처 맛집 찾기" (Google Maps 검색)
안내: "버튼을 누르면 새 창에서 주변 검색 결과가 열립니다."
```

**검색 URL:** `https://www.google.com/maps/search/?api=1&query=홍콩%20맛집`

---

### 시나리오 2: 키워드 검색 (관광지)

**입력:** "도쿄에서 관광지"

**기대 결과:**
```
확인 메시지: "도쿄 근처 **관광지** 검색 결과를 준비했습니다."
링크: "🔎 도쿄 근처 관광지 찾기" (Google Maps 검색)
```

**검색 URL:** `https://www.google.com/maps/search/?api=1&query=도쿄%20관광지`

---

### 시나리오 3: 일반 길찾기 (키워드 없음)

**입력:** "홍콩 공항에서 카이탁 터미널까지"

**기대 결과:**
```
확인 메시지: "확인했어요.\n출발지: 홍콩 공항\n도착지: 카이탁 터미널"
링크: 자동차, 대중교통, 도보 길찾기 (3개)
```

---

## 예상 효과

### 개선 전
- ❌ "홍콩에서 맛집" → 길찾기 링크만 제공
- ❌ 주변 검색 결과 없음
- ❌ 사용자 혼란

### 개선 후
- ✅ "홍콩에서 맛집" → 주변 맛집 검색 결과 제공
- ✅ Google Maps에서 여러 맛집 확인 가능
- ✅ 사용자 경험 개선

---

## 우선순위

### 🔴 최우선 (즉시 구현)

1. **키워드 감지 함수 확장** (30분)
   - `SEARCH_KEYWORDS` 목록 확장
   - `isKeywordSearch()` 함수 추가

2. **키워드 검색 핸들러 생성** (1시간)
   - `handleKeywordSearch()` 함수 구현
   - 검색 쿼리 생성 로직

3. **라우트 수정** (30분)
   - 키워드 검색 분기 추가
   - 기존 로직과 통합

**총 예상 시간:** 2시간

---

## 결론

### 문제 원인

1. ❌ **키워드 감지 로직 부재**: "맛집", "관광지" 등 키워드를 감지하지 않음
2. ❌ **검색 vs 길찾기 구분 없음**: 모든 입력을 길찾기로 처리
3. ❌ **검색 URL 미사용**: `gmSearchUrl()` 함수는 있으나 호출되지 않음

### 해결 방안

1. ✅ **키워드 감지 로직 추가**: `isKeywordSearch()` 함수 구현
2. ✅ **키워드 검색 핸들러 생성**: `handleKeywordSearch()` 함수 구현
3. ✅ **라우트 수정**: 키워드 검색 분기 추가

### 예상 효과

- ✅ "맛집", "관광지" 등 키워드 입력 시 주변 검색 결과 제공
- ✅ Google Maps에서 여러 결과 확인 가능
- ✅ 사용자 경험 대폭 개선

---

**작성자**: AI Assistant  
**최종 업데이트**: 2025-01-23

