            ...
            <div className="relative">
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2 border-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg transition-colors ${
                    isSubmitting ? 'opacity-60 cursor-not-allowed bg-gray-50' : 'border-gray-300 bg-white'
                  }`}
                />
                {purchaseSearchTerm && (
                  <button
...
export function createAdminAssignTripApi(): AssignTripApi {
  return {
    ...
    async fetchPurchaseCustomerTripInfo(userId: number) {
      const response = await fetch(`/api/admin/purchase-customers/${userId}/trip-info`, {
        credentials: 'include',
      });
      const data = await response.json();
...
export function createPartnerAssignTripApi(): AssignTripApi {
  return {
    async searchPurchaseCustomers(query: string) {
      ...
      const response = await fetch(`/api/partner/assign-trip/purchase-customers?${params.toString()}`, {
        credentials: 'include',
      });
      const data = await response.json();
      if (data.ok && Array.isArray(data.customers)) {
    ...
    async fetchPurchaseCustomerTripInfo(userId: number) {
      const response = await fetch(`/api/partner/assign-trip/purchase-customers/${userId}/trip-info`, {
        credentials: 'include',
      });
      const data = await response.json();
      if (!response.ok || !data.ok) {
