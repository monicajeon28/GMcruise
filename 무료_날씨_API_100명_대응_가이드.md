# 무료 날씨 API 100명 대응 가이드

## 🎯 목표
**100명 사용자까지 무료로 날씨 정보 제공**

---

## 📊 무료 플랜 비교

### WeatherAPI.com 무료 플랜
- ✅ **하루 1,000회** API 호출
- ✅ **14일 예보** 제공
- ✅ **무료 플랜에서도 상업적 사용 가능**
- ✅ **100명 사용자 충분**

### OpenWeather API 무료 플랜
- ✅ **월 100만 회** 호출 (하루 약 33,000회)
- ✅ **5일 예보만** 제공
- ⚠️ **상업적 사용 시 유료 플랜 필요** (무료는 개인/교육용)

---

## 💡 100명 사용자 호출량 계산

### 시나리오 1: 사용자별 개별 호출 (비효율적)
- 100명 × 하루 1회 = **하루 100회**
- WeatherAPI.com 무료: ✅ **충분** (1,000회 제한)
- OpenWeather 무료: ✅ **충분** (33,000회 제한)

### 시나리오 2: 지역별 캐싱 (권장) ⭐
- **같은 지역의 날씨는 1시간마다 1회만 호출**
- 예: 서울 날씨 → 1시간 캐시 → 하루 24회 호출
- 예: 도쿄 날씨 → 1시간 캐시 → 하루 24회 호출

**100명 사용자가 20개 지역을 사용한다고 가정:**
- 20개 지역 × 24회/일 = **하루 480회**
- WeatherAPI.com 무료: ✅ **충분** (1,000회 제한)
- OpenWeather 무료: ✅ **충분** (33,000회 제한)

---

## 🏆 최종 추천: WeatherAPI.com 무료 플랜

### 이유
1. ✅ **14일 예보** 제공 (OpenWeather는 5일만)
2. ✅ **하루 1,000회** 충분 (100명 + 캐싱)
3. ✅ **상업적 사용 가능** (OpenWeather는 유료 필요)
4. ✅ **무료로 시작 가능**

---

## 📝 구현 방법

### 1. WeatherAPI.com 계정 생성
1. https://www.weatherapi.com 접속
2. **Sign Up** (무료 계정)
3. **API Key** 발급 (Dashboard에서 확인)

### 2. Vercel 환경변수 설정
```
Key: WEATHER_API_KEY
Value: (WeatherAPI.com에서 발급받은 키)
Environment: All
```

### 3. 코드 구현

#### `lib/weather.ts` 생성
```typescript
// lib/weather.ts
export interface WeatherForecast {
  date: string;
  day: {
    maxtemp_c: number;
    mintemp_c: number;
    condition: {
      text: string;
      icon: string;
    };
  };
}

export interface WeatherResponse {
  location: {
    name: string;
    country: string;
  };
  current: {
    temp_c: number;
    condition: {
      text: string;
      icon: string;
    };
  };
  forecast: {
    forecastday: WeatherForecast[];
  };
}

/**
 * WeatherAPI.com을 사용하여 14일 날씨 예보 가져오기 (1시간 캐시)
 */
export async function getWeatherForecast(
  city: string,
  days: number = 14
): Promise<WeatherResponse | null> {
  try {
    const apiKey = process.env.WEATHER_API_KEY;
    
    if (!apiKey) {
      console.warn('[Weather] WEATHER_API_KEY가 설정되지 않았습니다.');
      return null;
    }

    // 1시간 캐시 적용 (Next.js 자동 캐싱)
    const response = await fetch(
      `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${city}&days=${days}&lang=ko`,
      { 
        next: { revalidate: 3600 } // 1시간 캐시
      }
    );

    if (!response.ok) {
      throw new Error(`날씨 API 요청 실패: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.error('[Weather] 날씨 정보 가져오기 실패:', error);
    return null;
  }
}
```

### 4. DailyBriefingCard 수정

`app/chat/components/DailyBriefingCard.tsx`에서 더미 데이터를 실제 API로 교체:

```typescript
import { getWeatherForecast } from '@/lib/weather';
import { useState, useEffect } from 'react';

// 컴포넌트 내부
const [weatherData, setWeatherData] = useState<WeatherResponse | null>(null);

useEffect(() => {
  if (briefing?.destination) {
    getWeatherForecast(briefing.destination, 14).then(setWeatherData);
  }
}, [briefing?.destination]);

// 날씨 표시 부분
{weatherData ? (
  <div>
    <p className="text-sm text-gray-600 mb-2">
      📍 {weatherData.location.name}, {weatherData.location.country}
    </p>
    <div className="grid grid-cols-7 gap-2">
      {weatherData.forecast.forecastday.map((day, index) => (
        <div key={index} className="text-center p-2 bg-white rounded-lg">
          <p className="text-xs text-gray-600">
            {new Date(day.date).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })}
          </p>
          <p className="text-2xl mb-1">{day.day.condition.icon}</p>
          <p className="text-sm font-bold">{Math.round(day.day.maxtemp_c)}°</p>
          <p className="text-xs text-gray-500">{Math.round(day.day.mintemp_c)}°</p>
          <p className="text-xs text-gray-600 mt-1">{day.day.condition.text}</p>
        </div>
      ))}
    </div>
  </div>
) : (
  <p className="text-sm text-gray-500">날씨 정보를 불러오는 중...</p>
)}
```

---

## 🎯 호출 최적화 전략

### 전략 1: 1시간 캐싱 (권장)
- 같은 지역의 날씨는 1시간 동안 캐시
- 20개 지역 × 24회/일 = **하루 480회**
- WeatherAPI.com 무료: ✅ **충분** (1,000회 제한)

### 전략 2: 3시간 캐싱 (더 안전)
- 같은 지역의 날씨는 3시간 동안 캐시
- 20개 지역 × 8회/일 = **하루 160회**
- WeatherAPI.com 무료: ✅ **여유있음** (1,000회 제한)

### 전략 3: 인기 지역 우선
- 인기 10개 지역: 1시간 캐시 = 240회/일
- 일반 10개 지역: 3시간 캐시 = 80회/일
- **총 320회/일**
- WeatherAPI.com 무료: ✅ **충분**

---

## 📊 비용 비교 (100명 기준)

### WeatherAPI.com
- **무료**: 하루 1,000회 ✅ **충분**
- **Starter ($4/월)**: 하루 10,000회 (불필요)

### OpenWeather
- **무료**: 월 100만 회 ✅ **충분**
- ⚠️ **상업적 사용 시 유료 필요**

**결론**: **WeatherAPI.com 무료 플랜으로 충분!**

---

## ✅ 체크리스트

### 즉시 시작 (무료)
- [ ] WeatherAPI.com 계정 생성
- [ ] API 키 발급
- [ ] Vercel 환경변수에 `WEATHER_API_KEY` 추가
- [ ] 코드 구현 (`lib/weather.ts` 생성)
- [ ] DailyBriefingCard 수정 (더미 데이터 → 실제 API)
- [ ] 테스트 완료

### 최적화 (선택)
- [ ] 1시간 캐싱 적용
- [ ] 지역별 호출 모니터링
- [ ] 호출량 추적 (하루 1,000회 이하 유지)

---

## 🚀 빠른 시작 가이드

### 1단계: API 키 발급 (5분)
1. https://www.weatherapi.com 접속
2. Sign Up → API Key 복사

### 2단계: 환경변수 설정 (2분)
1. Vercel → Settings → Environment Variables
2. `WEATHER_API_KEY` 추가
3. Redeploy

### 3단계: 코드 구현 (10분)
1. `lib/weather.ts` 파일 생성 (위 코드 복사)
2. `DailyBriefingCard.tsx` 수정 (더미 데이터 제거)
3. 테스트

**총 소요 시간: 약 20분**

---

## ⚠️ 주의사항

### 호출량 모니터링
- WeatherAPI.com 무료 플랜: 하루 1,000회 제한
- 초과 시 API 호출 실패
- Vercel 로그에서 호출량 확인

### 확장 시
- 100명 이상 사용 시 Starter 플랜 ($4/월) 고려
- 또는 OpenWeather 무료 플랜 검토 (5일 예보만)

---

## 🎯 결론

**100명까지 무료로 사용하려면: WeatherAPI.com 무료 플랜 사용**

**이유**:
1. ✅ **14일 예보** 제공
2. ✅ **하루 1,000회** 충분 (캐싱 적용 시)
3. ✅ **상업적 사용 가능**
4. ✅ **무료로 시작 가능**

**구현 시간**: 약 20분
**비용**: $0 (무료)

**지금 바로 시작하세요!** 🌤️


